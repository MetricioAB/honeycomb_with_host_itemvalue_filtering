...
$form
	...
	->addField(
		(new CWidgetFieldPatternSelectItemView($data['fields']['host_filter_items']))
			->setFilterPreselect($hostids_field !== null
				? [
					'id' => $hostids_field->getId(),
					'accept' => CMultiSelect::FILTER_PRESELECT_ACCEPT_ID,
			...
			->setFieldHint(makeHelpIcon(_('Hide all items from a host if any matching filter item has the specified latest raw value. Hosts without a recent filter value remain visible. Leave empty to disable.')))
	)
	->addField(
		new CWidgetFieldTextBoxView($data['fields']['host_filter_value'])
	)
	->addField(
