...
namespace Modules\Honeycomb_with_filter\Actions;
...
use Modules\Honeycomb_with_filter\Includes\WidgetForm;
...
class WidgetView extends CControllerDashboardWidgetView {
	...
	private function getCells(int $limit): array {
		...
		// Filter before sorting and limiting cells so excluded hosts do not consume display slots.
		$db_items = $this->filterHostItems($db_items);
	...
	private function filterHostItems(array $items): array {
		$patterns = $this->fields_values['host_filter_items'] ?? [];

		if (!$patterns || !$items) {
			return $items;
		}

		// Filter items are independent of the patterns and tags used for displayed cells.
		$filter_items = API::Item()->get([
			...
			'searchByAny' => true,
			'search' => [
				($this->isTemplateDashboard() ? 'name' : 'name_resolved') =>
					in_array('*', $patterns, true) ? null : $patterns
			],
			'filter' => [
				'status' => ITEM_STATUS_ACTIVE,
				'value_type' => [ITEM_VALUE_TYPE_FLOAT, ITEM_VALUE_TYPE_UINT64, ITEM_VALUE_TYPE_STR,
					ITEM_VALUE_TYPE_TEXT, ITEM_VALUE_TYPE_LOG]
			]
		]);

		$excluded_hostids = [];
		$expected = (string) ($this->fields_values['host_filter_value'] ?? '0');
		$history_period = timeUnitToSeconds(CSettingsHelper::get(CSettingsHelper::HISTORY_PERIOD));

		foreach (array_chunk($filter_items, self::MAX_ITEMS) as $batch) {
			$history = Manager::History()->getLastValues($batch, 1, $history_period);

			foreach ($batch as $item) {
				if (!isset($history[$item['itemid']][0]['value'])) {
					continue;
				}

				$value = (string) $history[$item['itemid']][0]['value'];
				$is_numeric = in_array($item['value_type'], [ITEM_VALUE_TYPE_FLOAT, ITEM_VALUE_TYPE_UINT64]);
				$matches = $is_numeric
					? is_numeric($expected) && is_numeric($value) && $value == $expected
					: $value === $expected;

				if ($matches) {
					$excluded_hostids[$item['hostid']] = true;
				}
			}
		}

		return array_values(array_filter($items,
			static fn(array $item): bool => !isset($excluded_hostids[$item['hostid']])
		));
	}
